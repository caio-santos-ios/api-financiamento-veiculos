...
namespace api_financiamento.src.Shared.Utils
{
    public class PaginationUtil<T>
    {
        ...
        public PaginationUtil(Dictionary<string, string> queries)
        {
            ...
            SortDefinition<T>? sortDefinition = sort.Equals(-1) ? Builders<T>.Sort.Descending(orderBy) : Builders<T>.Sort.Ascending(orderBy);
                ...
                if (!new List<string> { "pageSize", "pageNumber", "page", "sort", "orderBy" }.Contains(key))
                {
                    ...
                    switch (type)
                    {
                        case "bool":
                            ...
                            if (logic == "or")
                            {
                                if (pipelineFilter.Contains("$or")) pipelineFilter["$or"].AsBsonArray.Add(new BsonDocument(field, valueBool));
                                else pipelineFilter.Add("$or", new BsonArray { new BsonDocument(field, valueBool) });
                            }
                            break;
                        ...
                        case "int":
                            ...
                            if (logic == "or")
                            {
                                if (pipelineFilter.Contains("$or")) pipelineFilter["$or"].AsBsonArray.Add(new BsonDocument(field, valueNum));
                                else pipelineFilter.Add("$or", new BsonArray { new BsonDocument(field, valueNum) });
                            }
                            break;
                        ...
                        case "date":
                            ...
                            {
                                if (pipelineFilter.Contains("$or")) pipelineFilter["$or"].AsBsonArray.Add(new BsonDocument(field, valueDt));
                                else pipelineFilter.Add("$or", new BsonArray { new BsonDocument(field, valueDt) });
                            }
                            break;
                        ...
                        default:
                            ...
                            if (logic == "or")
                            {
                                if (pipelineFilter.Contains("$or")) pipelineFilter["$or"].AsBsonArray.Add(new BsonDocument(field, valueString));
                                else pipelineFilter.Add("$or", new BsonArray { new BsonDocument(field, valueString) });
                            }
                            break;
                    }
                }
